...
export default class BitskiTransfer {
   ...
   *   
   */
  async transferProps(ethNetwork: string, key: string, secret: string, to: string, amount: string) {
...
/**
   ...
   *   
   */
  async transferEth(ethNetwork: string, key: string, secret: string, to: string, amount: string) {
  ...
  // BITSKI
    ...
    await web3.eth.sendTransaction({ to, from: rewardsWallet, value: web3.utils.toWei(amount).toString(), gas: config.settings.ethereum.transfer_gas, gasPrice: web3.utils.toWei(gasPrice, 'gwei') })
       .then((receipt) => {
         const txHash = receipt.transactionHash;
         console.log(`Submitted ${txHash} receipt=${JSON.stringify(receipt)}, to:${to}, amount:${amount}`);                    
       }).catch((error) => {
