...
export default () => ({
  ...
  ethereum: {
    ...
    gas_price: process.env.GAS_PRICE ? Number(process.env.GAS_PRICE) : 480, // gwei
  ...
  activity: {
    state_rest_uri: process.env.STATE_API_URI ? process.env.STATE_API_URI : 'https://api.propsproject.io/v1/earnings/activity/daily_summary',
  },
  apidoc_users: {
    apidoc: {
      username: 'apidoc',
